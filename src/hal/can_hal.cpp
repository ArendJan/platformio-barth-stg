...
void FDCAN::init() {
  ...
  if (HAL_FDCAN_ActivateNotification(&can_handle, FDCAN_IT_RX_FIFO0_NEW_MESSAGE,
                                     0) != HAL_OK) {
    Error_Handler();
  }
...
void FDCAN::send(uint32_t id, uint8_t const *data,
                 size_t size) // only normal can for now
{
  ...
  if (HAL_FDCAN_AddMessageToTxFifoQ(&this->can_handle, &CAN_TxHeader,
                                    CAN_TxData) != HAL_OK) {
...
void MX_FDCAN1_Init(
    FDCAN_HandleTypeDef &hfdcan1) { // same as the MX_FDCAN2_Init below.
...
extern "C" void HAL_FDCAN_RxFifo0Callback(FDCAN_HandleTypeDef *hfdcan,
  ...
  if ((RxFifo0ITs & FDCAN_IT_RX_FIFO0_NEW_MESSAGE) != RESET) {
    ...
    if (HAL_FDCAN_GetRxMessage(hfdcan, FDCAN_RX_FIFO0, &CAN_RxHeader,
                               CAN_RxData) == HAL_OK) {
